...
## 2.9. 에디터 픽(editors_pick) 평가 (게이트)

`editors_pick`은 "편집자 추천" 배지를 다는 신호라 흔해지면 가치를 잃는다. 기본값은 **생략(false)**. 아래 3축 평가를 거친 후보만 사용자 승인을 받아 `true`로 설정한다.

### 3축, 각 5점 만점

**축 1. 효용성 (Usefulness)** — 비즈니스 리더가 얼마나 빨리·얼마나 큰 효용을 뽑는가
- 5: 설치 즉시 당일 업무에 투입, 반복 사용 명확
- 4: 설정 1~2단계 후 곧바로 가치
...
- **3축 평균 4.0 이상 → PICK 후보**. 과락(하한선) 규칙은 없음.
- 5점은 "이 카테고리에서 최상위 대안이 없다" 수준일 때만 부여. 인플레 금지.
- 판정이 애매해서 망설여지면 탈락 쪽으로 기울여라. "의심스러우면 PICK 아님"이 원칙.
- 이미 같은 카테고리에 PICK이 2개 이상이면 신규 PICK은 더 엄격하게(평균 4.3+ 권장) 판단.

### 절차

1. 2.7 게이트를 통과한 뒤, README·stars·저자·카테고리 현황을 근거로 3축 점수 산출.
2. 평균 4.0 미만이면 조용히 `editors_pick` 필드를 생략하고 3단계로. 사용자에게 굳이 보고하지 않아도 된다.
3. PICK 후보면 **중단**하고 아래 형식으로 사용자에게 승인 요청:
   ...
   editors_pick: true 로 추가할까요? (y/n)
   ```

4. `y` → 카드에 `"editors_pick": true` 포함. `n` → 필드 생략.
5. 사용자가 처음부터 `--pick` 또는 "에디터픽으로"라고 명시했어도 이 평가를 거쳐야 한다. 과락인데 강행 요청이면 이유를 한 번 역제안한 뒤 최종 결정은 사용자에게.
...
- `editors_pick`: 2.9 평가를 통과하고 사용자 승인을 받은 경우에만 `true`로 추가. 그 외는 필드 **생략**.
- `lang`: 사용자가 명시 요청한 경우에만 추가. 기본은 **넣지 말 것**.
